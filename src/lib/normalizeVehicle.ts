...
export function normalizeVehicleForUI(vehicle: Partial<Vehicle> & Record<string, any>) {
  const specsSrc: Record<string, any> = (vehicle.specs && typeof vehicle.specs === 'object') ? vehicle.specs : {};
  ...
  const rawFeatures: any[] = Array.isArray(vehicle?.features)
    ? vehicle.features
  ...
  const equipements: CarEquipmentItem[] = features.map((f) => ({
...
export default normalizeVehicleForUI;
